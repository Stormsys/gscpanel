...
<div id="content">
	<div id="server-info" data-gs-id="<?=$server->GetId()?>" data-status="<?=GameServerStatus::$STYLE_TEXT[$server->GetStatus()]?>">
		<input value="<?=$server->GetNickname()?>" size="<?=strlen($server->GetNickname()) + 1?>" class="mo-input h1"/>
		...
		<div class="actions-container">
			<a href="ftp://<?=$server->GetFtpUser()?>:<?=$server->GetFtpPass()?>@<?=$server->GetIp()?>:21" target="_blank" class="server-action-main">Connect FTP</a>
		</div>

		<div class="actions-container-r">
			<a  href="#/server/<?=$server->GetId()?>/reinstall" id="btn-reinstall"  class="server-action-main">Reinstall</a>
