<div class="content_pad">
	...
	<form class="admin_form" method="POST" action="/admin/gameserver-edit/<?=!isset($gs) ? 'new' : $gs->GetId();?>">
		<div><strong>ID</strong> <?=!isset($gs) ? 'new' : $gs->GetId();?> <?php if(isset($gs) && !empty($gs)):?>(<a href="/dashboard/manage/<?=$gs->GetId();?>">click here</a>  to stop/start/restart or reinstall the server.)<?php endif; ?></div>
		<label>
			<p>Nickname</p>
		...
		<label>
			...
			<select name="ds_id">
				...
				<?php foreach($dservers as $dserver): ?>
					<option value="<?=$dserver['dserver_id']?>" <?=(isset($gs) && $gs->GetDedicatedServerId() == $dserver['dserver_id']) || (!isset($gs) && isset($post['ds_id']) && $dserver['dserver_id']  == $post['ds_id']) ? 'selected' : '' ?>><?=$dserver['nickname']?></option>
				<?php endforeach; ?>
			</select>
		</label>
		...
		<label>
			...
			<select name="template_id">
				...
				<?php foreach($templates as $template): ?>
					<option value="<?=$template['template_id']?>" <?=(isset($gs) && $gs->GetTemplateId() == $template['template_id']) || (!isset($gs) && isset($post['template_id']) && $template['template_id'] == $post['template_id']) ? 'selected' : '' ?>><?=$template['long_name']?></option>
				<?php endforeach; ?>
			</select>
		</label>
		...
		<label>
			...
			<select name="owner-id">
				...
				<?php foreach($users as $user): ?>
					<option value="<?=$user['user_id']?>" <?=(isset($gs) && $gs->GetOwnerId() == $user['user_id']) || (!isset($gs)  && isset($post['owner-id']) && $user['user_id'] == $post['owner-id'])  ? 'selected' : '' ?>><?=$user['username']?></option>
				<?php endforeach; ?>
			</select>
		</label>
		...
		<label>
			<p>Startup CMD</p>
			<input name="paramater_overide" type="text" value="<?=!isset($gs) ?  $post['paramater_overide'] : $gs->GetCommandLine()?>" />
		</label>
		...
		<?php if(isset($gs) && !empty($gs)): ?>
			...
			<label>
				<p>Ftp Password</p>
				<input name="ftp_password" type="password" value="<?=!isset($gs) ?  $post['ftp_password'] : $gs->GetFtpPass()?>" />
			</label>
		<?php endif; ?>
